...
// 기존의 fstat 함수의 경우, File Discriptor를 인자로 받음.
// 이를 통해 stat 구조체에 파일과 관련된 Meta Data를 저장하도록 함.
// 구현과정에서 Symbolic Link 파일을 open 하였을 때, 원본파일이 열리도록 구현.
// 따라서, fstat으로 Symbolic Link 파일의 Meta Data를 확인할 수 없음.
// lstat System Call을 추가.
int
sys_fstat(void)
{
  struct file *f;
